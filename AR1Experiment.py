...
for i in range(nrep):
    ...
    wnh.run(lp,q0,step=nh.adaptNHstepE(),tp0=nh.NHtuningPars(Cmin=2,thermostatFac=25.0),niter=niter)
    res[k,0] = 2
    ...
    np.save("AR1Experiment", res)    
    
    #WALNUTS from here on
    ...
    wwnh.run(lp,q0,step=nh.adaptNHstepE(),tp0=nh.NHtuningPars(Cmin=2,thermostatFac=25.0),niter=niter)
    res[k,0] = 6
